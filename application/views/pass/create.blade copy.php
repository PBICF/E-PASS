                            ...
                            <div class="d-flex justify-content-between">
                                ...
                                <div class="form-groop rounded-2 bg-gradient p-2 flex-1 w-50">
                                    ...
                                    <input type="text" 
                                        ...
                                        autocomplete="off"
                                        x-model="oldPassNo"
                                        x-on:keydown.enter.prevent="onOldPassChange"
                                        x-on:keydown.tab.prevent="onOldPassChange">
                                </div>
                            </div>
                        </div>
                    </div>
        ...
        return {
            ...
            async submit() {
                ...
                if (this.currentStep === 3) {
                    const result = await swalAlert({
                        icon: 'info',
                        title: 'Confirm Pass Generation',
                        confirmButtonText: 'Yes, Generate',
                        message: 'Are you sure you want to generate the pass?',
                    });
            ...
            oldPassNo: '{{ old_input('old_passno') }}',
            employee: {
                empno: null,
                ename: '',
                desig: '',
            ...
            onOldPassChange() {
                ...
                fetch("{{ site_url('api/pass/details') }}", {
                    method: 'POST',
                    body: new URLSearchParams({ pass_number: this.oldPassNo })
                })
                .then(async res => {
                    const data = await res.json();
